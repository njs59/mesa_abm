...
Now with THREE conditions:
1) phase2_only
2) two_phase_interact (default behaviour)
3) two_phase_no_interact (NEW: cross-phase interactions disabled)
"""
...
# -----------------------------
# Worker task (modified)
# -----------------------------
def run_one_repeat_task(cond_name, phenotype, phase2_only,
                        allow_cross_phase, rep, steps, seed,
                        out_csv_str, base_params, n_init, init_size):
    ...
    # NEW: set cross-phase interaction rule
    params["interactions"]["allow_cross_phase_interactions"] = bool(allow_cross_phase)
    ...
    summary.update(dict(
        ...
        allow_cross_phase=allow_cross_phase,
...
def main():
    ...
    # --------------------------
    conditions = [
        Condition("phase2_only", "invasive", True,  True),
        Condition("two_phase_interact", "invasive", False, True),
        Condition("two_phase_no_interact", "invasive", False, False),  # NEW
    ]
    ...
    if workers == 1:
        for c_idx, cond, r, seed, out_csv_str in tasks:
            row = run_one_repeat_task(
                cond.name, cond.phenotype,
                cond.phase2_only, cond.allow_cross_phase,
    ...
    # Save summary table
    df_rep = pd.DataFrame(all_rows).sort_values(
        ["condition","repeat"]
