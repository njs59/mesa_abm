...
def main():
    ...
    parser.add_argument('--in', dest='in_csv', type=str, default='new_results/sweeps.csv')
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(args.outdir, 'basic_heatmap_mean_size_final.png'))

    fig2, ax2 = plt.subplots(figsize=(7, 4))
    for inv_speed, sub in df.groupby('invasive_speed'):
        means = sub.groupby('merge_prob')['n_final'].mean()
        ax2.plot(means.index, means.values, marker='o', label=f"inv_speed={inv_speed:.2f}")
    ...
    fig2.tight_layout()
    fig2.savefig(os.path.join(args.outdir, 'basic_lines_n_final_vs_merge.png'))
