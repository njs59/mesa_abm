...
def main():
    parser = argparse.ArgumentParser(description='Parameter sweeps for clusters ABM (basic)')
    ...
    parser.add_argument('--out_csv', type=str, default='new_results/sweeps.csv')
    ...
    for (merge_prob, inv_speed, prolif_speed, dt) in combos:
        for rep in range(args.seeds):
            ...
            rows.append({
                ...
                'dt': float(dt),
                'init_phenotype': str(args.init_phenotype),
                'rep': int(rep),
                **metrics,
            })
