...
DEFAULTS = {
    ...
    # Soft separation & fragment placement controls
    "physics": {
        "cell_volume": 1954.0,
        "density": 1.0,
    ...
    # Legacy / default movement controls (still honoured if no phases are set)
    "movement": {
        "mode": "constant",            # "constant" or "distribution"
        "direction": "isotropic",      # "isotropic" | "persistent" | "von_mises"
        ...
        # Only used if direction=="von_mises"
        "mu": 0.0,     # radians
        "kappa": 2.0,  # concentration (>=0); 0 -> uniform
    },

    # Optional, per-agent motion schedule. If provided, overrides "movement".
    # List is sorted by t_start; last with t_start <= model.time is active.
    # See README for examples.
    "movement_phases": None,

    # One merge probability parameter
    "merge": {
