...
DISTS = [
    uniform(loc=0.30, scale=0.70), # merge
    uniform(loc=0.005, scale=0.015), # prolif
    uniform(loc=0.0000, scale=0.0020), # frag
    uniform(loc=1.80, scale=0.30), # a
    uniform(loc=1.40, scale=2.20), # scale
    uniform(loc=0.0, scale=0.40), # kappa
    uniform(loc=0.0, scale=1.0),  # NEW dummy 0–1
]
...
def _apply_params_to_defaults(x):
    ...
    # Accept the dummy parameter but do not use it
    p_merge, prolif, frag, a_shape, a_scale, kappa, dummy = map(float, x)  # NEW
    ...
    # dummy is intentionally ignored
    return params
