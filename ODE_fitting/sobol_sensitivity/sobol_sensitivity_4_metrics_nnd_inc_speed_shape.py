...
from scipy.spatial import cKDTree               # for fast mNN
try:
...
PARAM_NAMES = [
    ...
    "movement_v2.phase2.speed_dist.params.shape",
...
DISTS = [
    ...
    uniform(loc=0.0000, scale=0.010), # frag_rate ∈ [0.0000, 0.010]
    uniform(loc=0.50, scale=2.00),     # a ∈ [0.50, 2.50]
    uniform(loc=1.01, scale=1.99),     # shape ∈ [1.01, 3.00]
...
# --------- spatial helper: median nearest-neighbour distance ----------
def _median_nn_distance(xy: np.ndarray) -> float:
    """
    Median nearest-neighbour distance for 2D points xy[N,2].
    Uses cKDTree (workers=-1) for speed. Returns 0.0 if fewer than 2 points.
    """
    ...
    if n < 2:
        return 0.0
    tree = cKDTree(xy)
    # k=2: self (0 distance) + nearest neighbour
    dists, _ = tree.query(xy, k=2, workers=-1)
    nn = dists[:, 1]
...
def _run_once(x, steps, seed, force_phase2):
    ...
    alive_agents = [a for a in model.agent_set if getattr(a, "alive", True)]
    sizes = np.array([float(a.size) for a in alive_agents], dtype=float)
    # positions for mNN (exclude None)
    xy = np.array([a.pos for a in alive_agents if a.pos is not None], dtype=float)
...
def run_sobol(
    ...
    if n_workers is None:
        try:
            # n_workers = max(1, (os.cpu_count() or 2) - 1)   # if you prefer CPU-1
            n_workers = max(1, (os.cpu_count() or 2))         # or use all logical CPUs
        except Exception:
            n_workers = 1
...
if __name__ == "__main__":
    ...
    res, boot, out = run_sobol(
        ...
        n_workers=None,       # auto: all logical CPUs in this template
        chunksize=16,
        adaptive=adaptive,
    )