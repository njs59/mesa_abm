...
Run from project root:
    python -m ODE_fitting.scripts.run_forward_sims_phase_2 \
        --start-step 71 --n-runs 100 --n-workers 8 --seed 42
"""
...
def run_simulation_worker(args):
    default_params, init_sizes, seed, start_step = args

    # Copy params to isolate parallel workers
    ...
    init_clusters = [{"size": int(s), "phenotype": "proliferative"}
                     for s in init_sizes]

    model = ClustersModel(params=params, seed=seed, init_clusters=init_clusters)

    # Force PHASE 2 only movement
...
# ------------------------------------------------------------
def parse_args():
    p = argparse.ArgumentParser(description="Parallel forward ABM simulation (Phase 2 only)")
...
def main():
    ...
    runs = np.stack(runs, axis=0)   # shape (n_runs, T, 3)
    T = runs.shape[1]
    ...
    p1 = plot_stat(0, "Number of Clusters (mean ± 95% CI)",
                   "Number of clusters", "plot_num_clusters.png")
    p2 = plot_stat(1, "Mean Cluster Size (mean ± 95% CI)",
                   "Mean cluster size", "plot_mean_cluster_size.png")
    p3 = plot_stat(2, "Mean Squared Cluster Size (mean ± 95% CI)",
                   "Mean squared size", "plot_mean_squared_cluster_size.png")
