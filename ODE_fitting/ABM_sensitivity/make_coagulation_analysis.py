...
def per_repeat_series(repeat_dir: Path, smooth_window: int) -> pd.DataFrame:
    ...
    # Return ONLY step + metrics
    return df[
        ["step", "P", "P_sq", "P_pairs", "P_smooth", "P_sq_smooth", "P_pairs_smooth"]
    ]
...
def aggregate_repeats(repeats: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    P_sq_cols = [c for c in merged.columns if c.startswith("P_sq_r")]
    P_pairs_cols = [c for c in merged.columns if c.startswith("P_pairs_r")]

    Ps_cols = [c for c in merged.columns if c.startswith("P_s_r")]
    Psq_cols = [c for c in merged.columns if c.startswith("P_sq_s_r")]
    Ppairs_cols = [c for c in merged.columns if c.startswith("P_pairs_s_r")]

    # Aggregate
    agg = pd.DataFrame(index=merged.index)
    agg["P_mean"] = merged[P_cols].mean(axis=1)
    agg["P_sq_mean"] = merged[P_sq_cols].mean(axis=1)
    agg["P_pairs_mean"] = merged[P_pairs_cols].mean(axis=1)

    agg["P_smooth_mean"] = merged[Ps_cols].mean(axis=1)
    agg["P_sq_smooth_mean"] = merged[Psq_cols].mean(axis=1)
    agg["P_pairs_smooth_mean"] = merged[Ppairs_cols].mean(axis=1)
...
def plot_1d_overlay_metric(
    ...
    """
    Overlay of smoothed curves for ANY metric.
    Same behaviour as plot_1d_overlay, but configurable.
    Produces one line per sweep value, coloured consistently.
    """
    ...
    # --- Colour mapping (identical to original function) -----------------------
    if is_num:
        numeric_vals = [float(v) for v in vals]
        vmin, vmax = min(numeric_vals), max(numeric_vals)
        norm = plt.Normalize(vmin=vmin, vmax=vmax)
...
def make_1d_sweep_gif_metric(scen_records, param, outdir, metric_col, ylabel):
    ...
    for rec in scen_records:
        ...
        series_map[v] = s
        vmin = float(np.nanmin(s.values)) if np.any(~np.isnan(s.values)) else np.inf
        vmax = float(np.nanmax(s.values)) if np.any(~np.isnan(s.values)) else -np.inf
