...
class ClustersModel(Model):
    ...
    def __init__(self, params=None, seed=42, init_clusters=None):
        ...
        # NEW: lightweight, controllable logging
        self.enable_logging = True  # can be turned off by runner for speed
        self.log_every = int(self.params.get("time", {}).get("log_every", 1))  # steps between logs
    ...
    def step(self):
        ...
        # NEW: conditional logging (skip entirely if disabled)
        if not self.enable_logging:
            return
        step_idx = int(self.time / max(1e-12, float(self.dt)))
        if self.log_every <= 1 or (step_idx % self.log_every == 0):
            self._log_append()

    def _log_append(self):
        # NEW: skip if logging disabled
        if not self.enable_logging:
            return
