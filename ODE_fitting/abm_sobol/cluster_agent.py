...
class ClusterAgent(Agent):
    ...
    def __init__(self, model, size, phenotype):
        # (Back‑/forward‑compatible super() call omitted for brevity — keep your original)
        try:
    ...
    def _soft_separate(self):
        ...
        if not neighs:  # NEW: quick exit if none
            return

        pos_self = np.asarray(p, dtype=float)
        ...
        for other in neighs:
            ...
            rij = pos_self - np.asarray(op, dtype=float)
            # NEW: squared distance first
            d2 = float(rij[0]*rij[0] + rij[1]*rij[1])
            ...
            if d2 < r_sum2:
                if d2 < 1e-24:
                    ...
                    d = float(np.sqrt(d2))   # sqrt only when needed
                    rij_unit = rij / d
                disp += (r_sum - d) * softness * rij_unit

        if disp[0] != 0.0 or disp[1] != 0.0:
            newp = pos_self + disp
            self.model.space.move_agent(self, (float(newp[0]), float(newp[1])))
    ...
    def _try_merge(self):
        ...
        contacts = []
        r_self = float(self.radius)
        r_self2 = r_self * r_self  # not used but kept for readability

        for other in neighbors:
            if other is self or not other.alive or other.pos is None:
                continue
            rij = np.asarray(other.pos, dtype=float) - pos_self
            d2 = float(rij[0]*rij[0] + rij[1]*rij[1])
    ...
    # (merge/proliferate/fragment unchanged from your version)
