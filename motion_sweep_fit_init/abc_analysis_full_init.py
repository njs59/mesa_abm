#!/usr/bin/env python3
"""
ABC Posterior Analysis (with init_n_clusters inferred)
- Loads pyABC databases via History.get_distribution
- Treats init_n_clusters as an ABC parameter
- Performs:
  * Final-time PPC (S0,S1,S2)
  * Time-series PPC (median, 5–95% band, spaghetti) aligned to observed t_start
  * Posterior identifiability (marginals, correlation, PCA, loadings)
  * Posterior summaries (weighted mean/sd, HPD)
  * IC sweeps and local sensitivity
  * Epsilon trajectory and ESS
Mesa version targeted: 2.2.4
"""
import argparse
...
# ABM imports (package path expected in working dir)
...
def particle_to_params(defaults, particle, motion, speed_dist):
    ...
    # init_n_clusters from ABC posterior
    if "init_n_clusters" not in particle:
        raise KeyError("ABC posterior missing init_n_clusters")
...
def analyze_file(db_path: Path,
    ...
    if obs_ts_df is not None and len(obs_ts_df) > 0:
        ...
        T = obs_ts.shape[0]
        total_steps = t_start + T  # Option B: simulate exactly required steps
        ...
        for j, i in enumerate(idxs):
            ...
            pmap = particle_to_params(DEFAULTS, particle, motion, speed_dist)
            ts = simulate_timeseries(pmap, total_steps=total_steps, start_step=t_start, seed=int(rng.integers(1, 2**31-1)))
            sims[j, :, :] = ts[:T, :]
        ...
        for k, s in enumerate(["S0","S1","S2"]):
            ...
            plt.plot(t, med[:,k], color="#1f77b4", lw=1.8, label="median")
            plt.plot(t, obs_ts[:,k], color="black", lw=1.5, label="observed")
            plt.xlabel("step"); plt.ylabel(s); plt.title(f"Time-series PPC — {s}")
            plt.legend(); plt.tight_layout(); plt.savefig(outdir / f"{db_path.stem}_PPC_ts_{s}.png"); plt.close()
        # discrepancy curves
        # integrated discrepancy per draw
        disc_per_draw = []
        for j in range(sims.shape[0]):
            d = np.sqrt(np.sum((sims[j,:,:] - obs_ts)**2, axis=1))  # per timepoint
    ...
    for p in params.columns:
        ...
        for v, tag in [(vm, 'minus'), (vp, 'plus')]:
            ...
            sim = run_abm(particle_to_params(DEFAULTS, mp.to_dict(), motion, speed_dist))
            d = np.sqrt((sim["S0"]-observed_final["S0"])**2 + (sim["S1"]-observed_final["S1"])**2 + (sim["S2"]-observed_final["S2"])**2)
...
# ---------------------- CLI ----------------------------
def main():
    ap = argparse.ArgumentParser(description="ABC posterior analysis (with init_n_clusters inferred)")
    ...
    # infer motion/speed_dist from filename (simple heuristic)
    for p in paths:
        name = p.stem
