...
class SmolModel(pints.ForwardModel):
    ...
    def _rhs(n0,t_curr,b,s,N):
        ...
        for i in range(0,N_max):
            ...
            if i == 0:
                ...
                splitting_gain = s * 2 * math.sqrt(2) * n0[1]
                for l in range(i+1,N_max):
                        splitting_gain += s * 2 * math.sqrt(l+1) * n0[l]
                ...
                splitting_loss = s * 2 * math.sqrt(i+1) * n0[i]
                splitting_gain = s * 2 * math.sqrt(i+2) * n0[i+1]            
                ...
                splitting_loss = s * 2 * math.sqrt(N_max) * n0[N_max-1]
    ...
    def simulate(self, parameters, times):
        ...
        if times[0] != 0:
            ...
            n_previous_times = odeint(SmolModel._rhs, n0, previous_times, (b,s,N))
            n_input = n_previous_times[-1,:]
            # print('N input', n_input)
            # print('Length', len(n_input))
        else:
            n_input = n0
        n = odeint(SmolModel._rhs, n_input, times, (b,s,N))
    ...
    def suggested_parameters(self):
        ...
        return np.array([0.0004, 0.0001, 100])
