...
# Plain (no-wrap) distances — matches experimental FOV metric
...
def compute_snapshot_summaries(model, *, stats_torus: bool = False) -> Dict[str, float]:
    ...
    stats_torus=False -> distances DO NOT wrap (FOV-like, recommended)
    stats_torus=True  -> distances use minimal-image torus (legacy)
    """
    # New ABM uses _alive_snapshot; fall back to legacy method name if needed
    if hasattr(model, "_alive_snapshot"):
        ids, pos, radii, sizes, speeds = model._alive_snapshot()
    else:
    ...
    if D.size == 0:
        ...
        # Simple g(r) estimator aligned with the same D matrix
...
def simulate_timeseries(model_factory, params: dict, total_steps: int, sample_steps: Tuple[int, ...]) -> np.ndarray:
    ...
    for step in range(total_steps + 1):
        if step in steps_set:
            s = compute_snapshot_summaries(m, stats_torus=False)  # NO WRAP for metric
