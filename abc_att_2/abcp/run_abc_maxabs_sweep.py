...
UPDATED FOR NEW ABM:
 - Single merge parameter (merge.p_merge)
 - Physics defaults (soft separation, fragment_minsep_factor)
 - Correct sweep behaviour
 - db_template used for BOTH sweep and single-run when provided
 - Backwards compatibility for legacy prob_contact_merge+adhesion
"""
...
# --------------------------------------------------------------------------------------
# Runtime patching for new ABM schema
# --------------------------------------------------------------------------------------

def _coerce_merge_params(params: dict) -> None:
    """
    Ensure params["merge"] contains ONLY 'p_merge'.
    Accepts legacy 'prob_contact_merge' and/or 'adhesion' and converts them into p_merge.
...
def _ensure_physics_defaults(params: dict) -> None:
    """
    Ensure physics defaults for Fix 1 (soft separation) & Fix 5 (fragment min-sep).
    """
...
def main():
    ap = argparse.ArgumentParser(description="ABC-SMC with MaxAbsScaler + L2 (patched)")
    ...
    ap.add_argument("--motion", type=str, default="isotropic",
                    help="'isotropic', 'persistent', comma-list, or 'all'")
    ap.add_argument("--speed", type=str, default="constant",
                    help="'constant', 'lognorm', 'gamma', 'weibull', comma-list, or 'all'")
    ...
    # ------------------------------------------------------------
    # SINGLE RUN — still use db_template if supplied
    # ------------------------------------------------------------
    if template:
        db_path = template.format(
