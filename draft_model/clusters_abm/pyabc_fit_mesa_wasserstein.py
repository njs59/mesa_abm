...
# pyabc_fit_mesa_wasserstein_scipy.py
# ABC–SMC for the Mesa ABM (persistent movement) using SciPy 1D Wasserstein distance
# across S0, S1, S2 with equal sample sizes. Includes 'init_cells'.
import argparse
...
def make_params_from_particle(defaults: Dict, particle: Dict[str, float]) -> Dict:
    """Build ABM params from an ABC particle with persistent movement.
    Adds 'init_cells' mapped to params['init']['n_clusters'] (cast to int).
    """
    params = {
...
# --- Model closure (align sim to FULL obs; enforce equal lengths) ---
...
# --- SciPy Wasserstein distance over S0,S1,S2 ---
def make_wasserstein_distance(obs: np.ndarray, weights: Tuple[float, float, float] = (1.0, 1.0, 1.0)):
    obs_S0, obs_S1, obs_S2 = obs[:, 0], obs[:, 1], obs[:, 2]
    w0, w1, w2 = [float(x) for x in weights]
    def distance(x: Dict, x0: Dict) -> float:
        if not (len(x["S0"]) == len(obs_S0) and len(x["S1"]) == len(obs_S1) and len(x["S2"]) == len(obs_S2)):
            raise RuntimeError("Wasserstein distance requires equal sample sizes for sim and obs segments.")
        d0 = wasserstein_distance(x["S0"], obs_S0)
        d1 = wasserstein_distance(x["S1"], obs_S1)
        d2 = wasserstein_distance(x["S2"], obs_S2)
        return w0 * d0 + w1 * d1 + w2 * d2
    return pyabc.distance.FunctionDistance(distance)

# --- CLI ---
def main():
    parser = argparse.ArgumentParser(description="ABC–SMC (SciPy Wasserstein) for Mesa ABM with persistent movement and init_cells.")
    ...
    parser.add_argument("--db_file", type=str, default="clusters_abm/results/pyabc_runs_wasserstein_scipy.db")
    parser.add_argument("--results_dir", type=str, default="clusters_abm/results")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--w_S0", type=float, default=1.0)
    parser.add_argument("--w_S1", type=float, default=1.0)
    parser.add_argument("--w_S2", type=float, default=1.0)
    ...
    prior = Distribution(
        speed_meanlog=RV("uniform", 0.5, 2.5 - 0.5),
        speed_sdlog=RV("uniform", 0.7, 3.0 - 0.7),
        heading_sigma=RV("uniform", 0.05, 0.5 - 0.05),
    ...
    dist_func = make_wasserstein_distance(obs, weights=(args.w_S0, args.w_S1, args.w_S2))
    ...
    print(f"pyABC SciPy Wasserstein run started. DB: {db_uri}")
    ...
    names = [
        "speed_meanlog", "speed_sdlog", "heading_sigma",
        "prolif_rate", "adhesion", "fragment_rate", "merge_prob", "init_cells",
    ]
    for name in names:
        ...
        print(f"{name:30s}: median={wq(0.5):.6g}, 5%={wq(0.05):.6g}, 95%={wq(0.95):.6g}")
