...
# pyabc_fit_mesa_wasserstein_scaled_no_init.py
# ABC–SMC for Mesa ABM using SciPy Wasserstein distance, scaled by observed IQR per stat.
# This version removes 'init_cells' from the parameter prior and fixes n_clusters from data (or a CLI override).
...
def make_params_from_particle(
    defaults: Dict,
    particle: Dict[str, float],
    fixed_n_clusters: int
) -> Dict:
    """
    Build ABM params from an ABC particle (persistent movement).
    'init_cells' is NOT a parameter here; we inject a fixed value for n_clusters.
    """
    params = {
    ...
    # Map remaining parameters (NO 'init_cells' here)
    mapping = {
    ...
    # Fix init phenotypes and n_clusters
    params["init"]["phenotype"] = "proliferative"
    params["init"]["n_clusters"] = int(max(1, round(fixed_n_clusters)))
    return params

# --- Model closure ---
def make_pyabc_model(obs: np.ndarray, start_step: int, base_defaults: Dict, rng: np.random.Generator, fixed_n_clusters: int):
    """
    fixed_n_clusters: integer used to set init.n_clusters for ALL simulations.
    """
    T_obs = obs.shape[0]
    def model(p: Dict) -> Dict:
        sim_params = make_params_from_particle(base_defaults, p, fixed_n_clusters=fixed_n_clusters)
...
# --- IQR-normalised Wasserstein distance ---
...
# --- CLI ---
def main():
    parser = argparse.ArgumentParser(description="ABC–SMC (IQR-scaled Wasserstein) for Mesa ABM, no init_cells parameter.")
    ...
    parser.add_argument("--db_file", type=str, default="clusters_abm/results/pyabc_runs_wasserstein_scaled_no_init.db")
    parser.add_argument("--results_dir", type=str, default="clusters_abm/results")
    parser.add_argument("--seed", type=int, default=42)
    # NEW: optional override for fixed n_clusters (if <= 0, infer from obs S0[0])
    parser.add_argument("--init_cells_fixed", type=int, default=0)
    ...
    # Decide fixed n_clusters: by default, use the observed S0 at the first timepoint
    ...
    model_func = make_pyabc_model(obs, args.start_step, DEFAULTS, rng, fixed_n_clusters=fixed_n_clusters)
