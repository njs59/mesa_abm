...
# pyabc_fit_mesa_mae.py
# ABC–SMC for the Mesa ABM (persistent movement) using MAE distance
# across S0, S1, S2 (normalised by FULL observed std). Includes 'init_cells'.
import argparse
...
# --- Model closure (align sim to FULL obs length) ---
...
# --- MAE distance over FULL obs ---
def make_mae_distance(obs: np.ndarray):
    obs_std = np.std(obs, axis=0, ddof=1) + 1e-12
...
# --- CLI ---
def main():
    parser = argparse.ArgumentParser(description="ABC–SMC (MAE) for Mesa ABM with persistent movement and init_cells.")
    ...
    parser.add_argument("--db_file", type=str, default="clusters_abm/results/pyabc_runs_mae.db")
    ...
    dist_func = make_mae_distance(obs)
    ...
    print(f"pyABC MAE run started. DB: {db_uri}")
