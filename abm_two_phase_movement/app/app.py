...
def SpaceView():
    ...
    agents_snapshot = list(m.agent_set)  # <- updated from m.agents
    fig = Figure()
    ax = fig.subplots()
    for a in agents_snapshot:
        ...
        phase_color = "yellow" if a.movement_phase == 1 else "deepskyblue"
        ax.add_patch(
            Circle(
                (x, y),
                radius=a.radius * 0.9,
...
def SizeHistogram():
    ...
    if m is None:
        return solara.Warning("Model not initialised.")
    sizes = np.array([a.size for a in list(m.agent_set) if a.alive and a.pos is not None])  # <- updated
    if sizes.size == 0:
...
def SummaryPlots():
    ...
    if len(size_log) == 0:
        ...
        sem_series = np.array([sizes.std(ddof=1) / np.sqrt(len(sizes)) if len(sizes) > 1 else 0])
    ...
    axs[1].fill_between(steps, mean_series - sem_series, mean_series + sem_series, alpha=0.2)
