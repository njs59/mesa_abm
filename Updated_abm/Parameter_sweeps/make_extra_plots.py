#!/usr/bin/env python3
"""
Extra plots for ABM parameter-sweep runs (PNG only), using early/mid/final snapshots.
...
def colour_mapping(series: pd.Series, numeric_threshold: float = 0.9):
    ...
    for cat in categories_sorted:
        ...
        handles.append(
            mpl.lines.Line2D([0], [0], marker="o", linestyle="", color=colour, label=str(cat), markersize=6)
        )
...
def main():
    ...
    parser.add_argument("--results-root", type=str, default=None,
                        help="Path to Parameter_sweeps_results. Default: sibling of this package.")
...
# ---- Wrapper so the sweep package's __main__ can call this directly ----
from pathlib import Path as _Path

def run_extra_plots(results_root: str, run: str = 'latest', *,
                    metrics_per_page: int = 10,
                    params_per_page: int = 4,
                    per_metric_params_per_page: int = 6) -> None:
    root = _Path(results_root)
    if not root.exists():
        print(f"[WARN] extra_plots: results root not found: {root}")
        return
    # Choose latest timestamped run if requested
    if run in (None, 'latest', 'LAST', 'Latest'):
        runs = sorted([p for p in root.iterdir() if p.is_dir()])
        if not runs:
            print('[WARN] No runs in results root for extra plots.')
            return
        run_dir = runs[-1]
    else:
        run_dir = root / run
    make_plots_for_run(run_dir,
                       metrics_per_page=metrics_per_page,
                       params_per_page=params_per_page,
                       per_metric_params_per_page=per_metric_params_per_page)