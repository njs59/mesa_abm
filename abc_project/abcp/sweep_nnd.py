...
------------------------------
1) **Per-stage overlays for ALL stats** (NND, S0, S1, S2) after each stage.
2) **Final-stage uncertainty bands**: after the last stage, the script takes the
   best parameter vector, runs many fresh simulations, and plots **95% CIs**
   (and median) against the observed series for **NND, S0, S1, S2**.
   - Controlled by `--final_ci_replicates` and `--ci_seed`.
   - Saves PNG figures and a CSV of the full ensemble.
...
def save_final_uncertainty(best_vec: dict, timesteps: List[int], obs_mat: np.ndarray, stats_idx: dict,
    ...
    """Run many sims at the best parameters and save 95% CI overlays + CSV ensemble."""
    T = len(timesteps)
    K = 4  # S0,S1,S2,NND_med
    sims = np.zeros((ci_reps, T, K), dtype=float)
    for r in range(ci_reps):
        sim = simulate_timeseries(
            params_from_vector(best_vec, direction, speed),
...
def parse_cli():
    ap = argparse.ArgumentParser(description="Multi-stage NND sweep with per-stage overlays and final 95% CI")
...
def main():
    ...
    # NEW: Final uncertainty bands (95% CI) at best parameters
