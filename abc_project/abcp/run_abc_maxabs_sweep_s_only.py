...
- NEW: --s_only flag to fit only S0,S1,S2 (exclude NND_med and all g_r*).
- NEW: tqdm progress bar over sweep combinations (falls back to prints if tqdm is absent).
"""
...
# Optional: tqdm for sweep progress bar
try:
...
def run_one(
    ...
    # The pyABC sampler prints a per-population progress bar when show_progress=True
...
def main():
    ...
    ap.add_argument("--no_gr", action="store_true",
                    help="Exclude g_r40 and g_r80 from the fitted summary set.")
    ap.add_argument("--s_only", action="store_true",
                    help="Fit only to S0,S1,S2 (exclude NND_med and all g_r*).")
    ...
    if args.s_only:
        ...
        # Keep NND_med but drop radial growth summaries
    ...
    if do_sweep:
        ...
        if db_template is None or db_template.strip() == "":
            ...
            suffix = "_Sonly" if args.s_only else ("_noGR" if args.no_gr else "")
    ...
    # Run
    if do_sweep:
        combos = [(m, s) for m in motion_list for s in speed_list]
        iterator = tqdm(combos, desc="Sweep", unit="combo") if _HAS_TQDM else combos

        combo_idx = 0
        for motion, speed in iterator:
            seed = args.seed + combo_idx * max(1, args.seed_increment)
            suffix = "_Sonly" if args.s_only else ("_noGR" if args.no_gr else "")
            db_path = db_template.format(motion=motion, speed=speed)
            # If user forgot to encode suffix in template, append here
            ...
            if not _HAS_TQDM:
                print(f"\n[run {combo_idx+1}/{len(combos)}] motion={motion} speed={speed} seed={seed}")
                print(f"[db ] {db_path}")
        ...
        # Single run
        if not _HAS_TQDM:
            print(f"\n[run] motion={motion_list[0]} speed={speed_list[0]} seed={args.seed}")
            print(f"[db ] {args.db}")
